...
final class FundingSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $stadtfoerderung = Funding::create([
            ...
            'approved_amount'       => 250000, // 2.500 €
            'funding_period_start'  => "{$lastYear}-01-01",
            'funding_period_end'    => "{$lastYear}-12-31",
        ]);

        // Zwei Tranchen erhalten
        $this->fundingIncome($stadtfoerderung, 'Stadtförderung Vereinsheim – 1. Tranche', 150000, "{$lastYear}-04-01");
        $this->fundingIncome($stadtfoerderung, 'Stadtförderung Vereinsheim – 2. Tranche', 100000, "{$lastYear}-07-15");
        ...
        $jugendfoerderung = Funding::create([
            ...
            'approved_amount'       => 180000, // 1.800 €
            'funding_period_start'  => "{$lastYear}-09-01",
            'funding_period_end'    => "{$currentYear}-08-31",
        ]);

        // Erste Tranche bereits erhalten
        $this->fundingIncome($jugendfoerderung, 'BJR Jugendförderung – Bewilligungsbescheid Abschlag', 90000, "{$lastYear}-09-15");

        // Verknüpfung mit Jugendprogramm (Teilfinanzierung)
        if ($youth) {
            $jugendfoerderung->projects()->attach($youth->id, [
                'allocated_amount' => 120000, // nur Teilbetrag des Projekts wird gefördert
            ]);
        }
        ...
        $bfd = Funding::create([
            ...
            'status'                => FundingStatus::Completed,
            'approved_amount'       => 48000, // 480 €
            'funding_period_start'  => ($lastYear - 1).'-01-01',
            'funding_period_end'    => ($lastYear - 1).'-12-31',
        ]);

        $this->fundingIncome($bfd, 'BAFzA BFD-Zuschuss – Jahrespauschale', 48000, ($lastYear - 1).'-06-01');
