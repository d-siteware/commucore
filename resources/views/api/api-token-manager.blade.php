<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('auth.api_tokens.create') }}
        </x-slot>

        <x-slot name="description">
            {{ __('auth.api_tokens.description') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('auth.api_tokens.token_name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('auth.api_tokens.permissions') }}" />
        ...
        <x-slot name="actions">
            <x-action-message class="me-3" on="created">
                {{ __('auth.api_tokens.created') }}
            </x-action-message>

            <x-button>
                {{ __('auth.api_tokens.create_btn') }}
            </x-button>
        </x-slot>
    </x-form-section>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('auth.api_tokens.manage') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('auth.api_tokens.manage_description') }}
                </x-slot>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        <x-slot name="title">
            {{ __('auth.api_tokens.api_token') }}
        </x-slot>

        <x-slot name="content">
            <div>
                {{ __('auth.api_tokens.copy_token') }}
            </div>
    ...
    <x-dialog-modal wire:model.live="managingApiTokenPermissions">
        <x-slot name="title">
            {{ __('auth.api_tokens.token_permissions') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('auth.api_tokens.delete_token') }}
        </x-slot>

        <x-slot name="content">
            {{ __('auth.api_tokens.delete_confirm') }}
        </x-slot>
